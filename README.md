...
## reads
Most models have implicit reads, writes, and parsers.
...
in the sequence that was passed in. The key here is the final part of the method chain: ```scala .as(Project.parser.*)```. This is
...
The main differences? We are using the table name as a prefix, and the inclusion of the ```scala UserBase.userParser.? ``` line and the
```scala CommentItem.parser.? ``` line. All these are are nested row parsers for the JOINed tables. The '?' at the end just means that they can
